...
def _run(rank, world_size, cfg):
    ...
    # ---- conditional eval helpers (used at eval_freq and snapshot_freq) ----
    def conditional_generate(batch_ids, batch_mask):
        """EMA-weighted conditional sampling: pin prompt+padding, fill response."""
        prompt_mask = (batch_mask == 0)

        def proj_fun(x):
            x[prompt_mask] = batch_ids[prompt_mask]
            return x

        sampling_fn = sampling.get_pc_sampler(
            graph, noise, batch_ids.shape,
    ...
    def grade_samples(sample, batch_mask, answers):
        """Decode each response region and grade with r1_zero_reward_fn."""
        fmt_sum = ans_sum = 0.0
        n_graded = 0
        rows = []
        for idx in range(sample.shape[0]):
            resp_text = tokenizer.decode(sample[idx][batch_mask[idx].bool()])
            ...
            if HAS_GRADER and gt_answer:
                ...
                ans_sum += ans_r
                n_graded += 1
            rows.append((resp_text, gt_answer, fmt_r, ans_r))
        return fmt_sum, ans_sum, n_graded, rows

    def log_accuracy(step, fmt_sum, ans_sum, n_graded):
        """All-reduce per-rank sums and log format/answer accuracy (rank 0)."""
    ...
    while state['step'] < num_train_steps + 1:
        ...
        if step != state['step']:
            ...
            # generation produced at eval_freq, cached for the snapshot dump
            gen_cache = None

            if step % cfg.training.eval_freq == 0:
                eval_token_ids, eval_response_mask, eval_answers = next(eval_iter)
                ...
                # format/answer accuracy via conditional generation (every eval_freq)
                if cfg.training.snapshot_sampling:
                    sample = conditional_generate(eval_token_ids, eval_response_mask)
                    fmt_sum, ans_sum, n_graded, rows = grade_samples(
                        sample, eval_response_mask, eval_answers)
                    if HAS_GRADER:
                        log_accuracy(step, fmt_sum, ans_sum, n_graded)
                    else:
                        mprint(f"grader unavailable ({_GRADER_IMPORT_ERR}); "
                               f"skipping format/answer accuracy.")
                    # reuse this generation for the snapshot sample dump below
                    gen_cache = (sample, eval_token_ids, eval_response_mask, rows)
            ...
            if step > 0 and step % cfg.training.snapshot_freq == 0 or step == num_train_steps:
                ...
                # Dump conditional (SFT) samples. Reuse the eval-step generation
                # when available (eval_freq divides snapshot_freq), else generate.
                if cfg.training.snapshot_sampling:
                    mprint(f"Dumping conditional samples at step: {step}")
                    ...
                    if gen_cache is None:
                        ...
                        sample = conditional_generate(cond_token_ids, cond_response_mask)
                        _, _, _, rows = grade_samples(sample, cond_response_mask, cond_answers)
                        gen_cache = (sample, cond_token_ids, cond_response_mask, rows)

                    sample, cond_token_ids, cond_response_mask, rows = gen_cache
                    ...
                    with open(file_name, 'w', encoding="utf-8") as file:
                        for idx, (gen, gt) in enumerate(zip(gen_sentences, gt_sentences)):
                            resp_text, gt_answer, fmt_r, ans_r = rows[idx]
