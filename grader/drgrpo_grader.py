...
# --- optional heavy deps (require Python >=3.10) ---------------------------
# These are ONLY needed by is_latex_equal(), i.e. the fast=False recall path.
# With fast=True (our default) the grader works on sympy alone, so we import
# them lazily and degrade gracefully when they are unavailable (e.g. py3.9).
try:
...
try:
    ...
    ExprExtractionConfig = LatexExtractionConfig = parse = verify = None
...
def is_latex_equal(given_answer: str, ground_truth: str) -> bool:
    if not _HAS_MATH_VERIFY:
        # math_verify unavailable (e.g. Python < 3.10); fast=False recall is
        # disabled. fast=True grading does not call this path.
        return False
    try:
        with timeout(1):
            try:
